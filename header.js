(function(){
  ...
  function render(){
    ...
    html+='<div class="sh-deploy" id="'+uid+'"></div>';
    if(title){
      html+='<h1>'+title+'</h1>';
    }
    el.innerHTML=html;
    ...
    fetch(deployPath+'?v='+Date.now())
      ...
      .then(function(d){
        ...
        if(node)node.textContent=dt.getUTCFullYear()+'-'+pad(dt.getUTCMonth()+1)+'-'+pad(dt.getUTCDate())+' '+pad(dt.getUTCHours())+':'+pad(dt.getUTCMinutes())+' GMT+1';
      }).catch(function(){});
  }
